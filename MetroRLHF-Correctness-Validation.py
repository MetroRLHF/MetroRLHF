...
DTYPE=float32 T=512  python3 ./MetroRLHF-Correctness-Validation.py
DTYPE=float32 T=4096 python3 ./MetroRLHF-Correctness-Validation.py
DTYPE=float32 T=0    python3 ./MetroRLHF-Correctness-Validation.py

DTYPE=bfloat16 T=512  python3 ./MetroRLHF-Correctness-Validation.py
DTYPE=bfloat16 T=4096 python3 ./MetroRLHF-Correctness-Validation.py
DTYPE=bfloat16 T=0    python3 ./MetroRLHF-Correctness-Validation.py
'''
...
total_steps = int(os.environ.get('STEP', 10))
max_length = 8192
batch_size = 1
...
blockSize = int(os.environ.get('T', 1024))
...
def register_data_sample(question, answer, system='You are a helpful assistant.'):
    ...
    assert answer_ending > answer_begin, 'Response section end not found for this input sequence.'
...
class Qwen3(torch.nn.Module):
    ...
    def lora_weight(self, w, lo_rank=32):
        if lo_rank == 0:
...
params = filter(lambda p: p.requires_grad, graph.parameters())
optimizer = Optim(params, lr=default_lr)
trainable_parameters_num = sum(p.numel() for p in filter(lambda p:p.requires_grad, graph.parameters()))  ##全参微调

graph.train()
graph.zero_grad()

data_inputs = [
  register_data_sample('Music Sequence:', '1234567|' * 1000),
]

for iters in range(total_steps):
    data_sample = data_inputs[iters % len(data_inputs)]
    chosen_x = data_sample['QA'].flatten()[:-1].cuda()
    chosen_y = data_sample['QA'].flatten()[1:].cuda()
    answer_mask = torch.arange(0, data_sample['QA'].numel(), dtype=torch.int32, device=chosen_x.device)
    answer_mask = torch.logical_and(answer_mask >= data_sample['answer_begin'], answer_mask < data_sample['answer_ending'])[1:]
    ...
    for B in reversed(range(x_blocks.size(1))):
        ...
        K = torch.log_softmax(chosen_logits, dim=-1) / gradient_accumulation_steps
        K = (-torch.gather(K, dim=-1, index=Y.unsqueeze(2))).flatten() * MASK.flatten()

        loss = K.sum() / total_actions
        losses += [loss.detach().flatten()]
        # loss.register_hook(fn)
        loss.backward()
        # t2 = autort.wait()
        # print('train_step', t2 - t1)
        skip_block = True
    ...
    for i, c in enumerate(reversed(grad_collections)):
      print(f'>> Blockwise sequence S_{{{i * processedBlockSize} .. {(i + 1) * processedBlockSize - 1}}}:')
      print(torch.round(c, decimals=4))
      print()

    print(f'>> Iters:{iters+1}, dtype={dtype}, cost={time.perf_counter() - last_ts:.2f} sec, lr: {lr}, loss:{torch.cat(losses).sum().item():.4f}, max_reserved_GPU_memory:{torch.cuda.max_memory_reserved() / (1024**2):.2f} MB')
